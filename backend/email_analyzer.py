...
THREAT_KEYWORDS = [
    ...
    "unauthorized login", "your account has been compromised", "fraudulent activity detected",
    "pay now", "act immediately", "your service will be terminated", "login attempt detected",
    "security breach", "confirm identity", "access restricted", "action required", "emergency",
    "you are in violation", "locked account", "final warning", "report to authority", "criminal charges",
    "your device is infected"
]

SENSITIVE_KEYWORDS = [
    "otp", "password", "passcode", "ic number", "bank account", "credit card", "login", "verify",
    "security question", "ssn", "cvv", "nric", "identity card", "account number", "pin", "sort code",
    "routing number", "dob", "mother's maiden name", "social security", "personal info", "credentials",
    "2fa code", "mobile number", "transaction pin", "verification code"
]

URL_SHORTENERS = [
    'bit.ly', 't.co', 'goo.gl', 'tinyurl.com', 'ow.ly', 'buff.ly', 'is.gd', 'cutt.ly',
    'rb.gy', 'rebrand.ly', 'shorte.st', 'adf.ly', 'bl.ink', 'lnkd.in', 'chilp.it', 'soo.gd',
    'trib.al', 'tiny.cc', 'shorturl.at', 'clicky.me', 'vur.me', 'snip.ly'
]
SUSPICIOUS_TLDS = [
    '.tk', '.ml', '.ga', '.ru', '.cn', '.xyz', '.top', '.loan',
    '.click', '.gq', '.cf', '.work', '.support', '.zip', '.cam', '.men',
    '.mom', '.party', '.review', '.accountant', '.science', '.stream', '.win',
    '.info', '.biz', '.rest', '.country', '.download', '.racing', '.faith', '.date',
    '.cricket', '.link', '.hosting', '.porn', '.sex', '.exe'
]
...
def analyze_email(content: str, sender: str = "") -> Dict[str, Any]:
    ...
    flags = {
        ...
        'creates_urgency': any(word in text_lower for word in ['urgent', 'immediately', 'warning', 'alert', 'now', 'quick', 'fast']),
        'no_personal_greeting': not any(line.lower().startswith(greeting) for greeting in ['dear', 'hi', 'hello', 'greetings'] for line in lines[:5]),
    ...
    risk_level = (
        "High" if risk_percentage > 70 or num_flags >= 4 else
        "Medium" if risk_percentage > 40 or num_flags >= 2 else
        "Low"
    )
    ...
    if flags['has_threatening_language'] and threat_found:
        explanations['has_threatening_language'] = f"Detected threatening keyword(s): {', '.join(threat_found)}."
    if flags['asks_sensitive_info'] and sensitive_found:
        explanations['asks_sensitive_info'] = f"Sensitive keyword(s) found: {', '.join(sensitive_found)}."
    if flags['creates_urgency']:
        explanations['creates_urgency'] = "Message contains urgency phrases like 'urgent', 'now', or 'immediately'."
    if flags['no_personal_greeting']:
        explanations['no_personal_greeting'] = "Greeting is generic like 'Dear user' instead of using your name."
    ...
    if flags['grammar_errors']:
        explanations['grammar_errors'] = "Message may contain grammar issues due to word count or structure."
    if flags['contains_links']:
        explanations['contains_links'] = f"The message contains {len(links)} link(s), which may be used for phishing."
    if flags['suspicious_attachments'] and suspicious_extensions_found:
        explanations['suspicious_attachments'] = f"Suspicious file extension(s) mentioned: {', '.join(suspicious_extensions_found)}."
    ...
    if sender:
        ...
        if sender_info["suspicious_tld"] or sender_info["spoofing_brand"] or sender_info["too_many_subdomains"]:
            flags["suspicious_sender"] = True
            explanations["suspicious_sender"] = "Sender email looks suspicious based on domain and branding."
    ...
    try:
        ...
        if phonenumbers.is_possible_number(parsed) and phonenumbers.is_valid_number(parsed):
            if phonenumbers.number_type(parsed) == phonenumbers.PhoneNumberType.MOBILE:
                ...
                return "Landline"
            elif phonenumbers.number_type(parsed) == phonenumbers.PhoneNumberType.TOLL_FREE:
                return "Toll-Free"
            elif phonenumbers.number_type(parsed) == phonenumbers.PhoneNumberType.SHORT_CODE:
                return "Short Code"
            else:
                return "Other"
        else:
...
def analyze_sms(content: str, sender: str = "") -> Dict[str, Any]:
    ...
    risk_level = (
        "High" if risk_percentage > 70 or num_flags >= 3 else
        "Medium" if risk_percentage > 40 or num_flags >= 1 else
        "Low"
    )
    ...
    if sender:
        ...
        if sender_info["phone_type"].lower() in ["premium", "shortcode", "invalid"]:
            flags["suspicious_sender"] = True
            explanations["suspicious_sender"] = f"Sender number type '{sender_info['phone_type']}' is commonly used in scams."
...
def analyze_url(url: str) -> Dict[str, Any]:
    ...
    flags = {
        ...
        'contains_suspicious_keywords': any(kw in url.lower() for kw in ['secure', 'login', 'verify', 'bank', 'account', 'update', 'confirm']),
    ...
    risk_level = (
        "High" if risk_percentage > 70 or num_flags >= 2 else
        "Medium" if risk_percentage > 40 or num_flags >= 1 else
        "Low"
    )
    ...
    if flags['suspicious_tld']:
        explanations['suspicious_tld'] = f"The domain ends with a suspicious TLD like {domain[-4:]}."
    if flags['is_url_shortener']:
        explanations['is_url_shortener'] = f"The domain '{domain}' is a known URL shortener."
    if flags['contains_suspicious_keywords']:
        explanations['contains_suspicious_keywords'] = "The URL contains keywords like 'login', 'bank', or 'verify'."
    if flags['excessive_subdomains']:
        explanations['excessive_subdomains'] = "The domain has many subdomains which may indicate phishing."
