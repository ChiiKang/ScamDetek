...
const ScamDetection = () => {
  ...
  const renderAnalysisPanel = () => {
    if (isAnalyzing) {
      return (
        <div className="w-1/2 flex flex-col items-center justify-center py-10">
          ...
          <p className="text-white text-opacity-80">Analyzing your content...</p>
        </div>
      );
    }
    ...
    return (
      <div className="w-1/2 bg-opacity-30 bg-[#282828] rounded-lg overflow-hidden border border-opacity-20 border-[#4FD1C5] max-h-[650px] overflow-y-auto">
        ...
        <div className="p-5">
          <div className="mb-6">
            <h4 className="text-[#4FD1C5] mb-4 text-base font-normal mt-0">Warning Signs</h4>
          ...
          <div>
            <h4 className="text-[#4FD1C5] mb-4 text-base font-normal mt-0">Detected Traits</h4>
            <div className="flex flex-col gap-2.5">
              {Object.entries(analysisResult.metadata).map(([key, value]) => {
                ...
                if (Array.isArray(value)) {
                  ...
                  formattedValue = typeof value === 'boolean' ? (value ? "Yes" : "No") : value;
                }
                ...
                return (
                  ...
                  >
                    <span className="font-semibold text-[#4FD1C5] mr-1.5">{formattedKey}:</span>
                    <span className="text-white text-opacity-90">{formattedValue}</span>
                  </div>
                );
              })}
            </div>
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto">
      <div className="flex bg-[#282828] bg-opacity-30 rounded-[20px] overflow-hidden min-h-[700px]">
        ...
        {/* Main content */}
        <div className="flex-1 p-7 flex gap-6">
          {/* Input section */}
...
const simulateAnalysisResult = (contentType, content) => {
  ...
  const hasUrgentLanguage = /urgent|immediately|warning|alert|now|act now|hurry/i.test(lowerContent);
  const hasFinancialTerms = /bank|account|credit|debit|money|payment|transfer|financial/i.test(lowerContent);
  const hasSensitiveRequests = /password|verify|otp|pin|authenticate|login|credentials|confirm/i.test(lowerContent);
  const hasSuspiciousLinks = /http|www|\.com|\.net|click/i.test(lowerContent);
  const hasThreats = /suspend|disable|close|terminate|legal|police|arrest|criminal/i.test(lowerContent);
  ...
  const result = {
    ...
    flags: {
      ...
      no_personal_greeting: !lowerContent.includes("dear") && contentType === "email",
      url_mismatch: contentType === "email" && Math.random() > 0.7 && hasSuspiciousLinks
    },
    metadata: {
  ...
  if (contentType === "email") {
    ...
    if (content.includes("@")) {
      result.metadata.sender_domain = content.match(/@([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})/)?.[1] || "unknown";
    }
    
    if (hasSensitiveRequests) {
      result.metadata.sensitive_keywords_found = ["password", "verify", "confirm"].filter(
        word => lowerContent.includes(word)
      );
    }
    
    if (hasThreats) {
      result.metadata.threat_keywords_found = ["urgent", "warning", "suspend", "legal"].filter(
        word => lowerContent.includes(word)
      );
    }
  } else if (contentType === "url") {
    result.metadata.domain = content.match(/(?:https?:\/\/)?(?:www\.)?([^\/]+)/i)?.[1] || content;
    result.metadata.uses_https = content.startsWith("https://");
    result.metadata.suspicious_tld = /\.(tk|ml|ga|gq|cf|xyz|top|loan|info)$/i.test(content);
  }
