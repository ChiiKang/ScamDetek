...
const ScamDetection = () => {
  ...
  const senderFlagDescriptions = {
    ...
    free_provider_impersonating_brand: "Free email pretending to be a legit brand.",
    too_many_subdomains: "Looks for long domains that might hide real identity."
  };
        ...
        <div className="placeholder-content">
          ...
          <div style={{ marginTop: "32px", opacity: 0.7 }}>
            <div style={{ display: "flex", alignItems: "center", marginBottom: "12px" }}>
              ...
              <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>High Risk</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", marginBottom: "12px" }}>
              <span style={{
                display: "inline-block",
              ...
              <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Medium Risk</span>
            </div>
            <div style={{ display: "flex", alignItems: "center" }}>
              ...
              <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Low Risk</span>
            </div>
          </div>
        </div>
      );
            ...
            <div className="flags-column">
              ...
              <ul className="flags-list">
                {Object.entries(analysisResult.flags).map(([key, value]) => {
                  ...
                  const explanation = analysisResult.explanations?.[key] || "This may indicate scam behavior.";
            ...
            <div className="metadata-column">
              ...
              <div className="metadata-list">
                {Object.entries(analysisResult.metadata).map(([key, value]) => {
                  if (key === 'sender_analysis' || value === null || value === undefined) return null;
                      ...
                      <table className="metadata-table">
                        ...
                        <tbody>
                          {Object.entries(value).map(([subKey, subVal], index) => (
                            <tr key={index}>
                              <td>{subKey.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</td>
                              <td>
                                {subVal === true ? "✅ Yes" : subVal === false ? "❌ No" : String(subVal)}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                ...
                {analysisResult.metadata?.sender_analysis && (
                  <div className="sender-analysis-column">
                    ...
                    <ul className="sender-analysis-list">
                      {Object.entries(analysisResult.metadata.sender_analysis).map(([key, value], index) => {
                        ...
                        if (key === "phone_type") {
                          return (
                            <li key={index} className="sender-analysis-item">
                              <span className="sender-analysis-key">{formattedKey}</span>
                              <span className="sender-analysis-status" style={{ color: "#4FD1C5" }}>
                                {value}
                              </span>
                            </li>
                          );
                        ...
                        const isFlagged = key === "is_valid_format" ? !value : value;
                        ...
                        return (
                          <li
                            ...
                            >
                            <span className="sender-analysis-key">{formattedKey}</span>
                            <span className="sender-analysis-status" style={{ color: statusColor }}>
