...
const ScamWordCloud = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    import('d3-cloud').then((cloudModule) => {
      ...
      function draw(words) {
        ...
        const color = d3.scaleOrdinal()
          ...
          .range([
            ...
            "#81E6D9", // Lighter teal
            "#ffffff", // White
            "#000000", // Black (changed from yellow)
      ...
      console.error("Error loading d3-cloud:", error);
      // Add fallback display when d3-cloud fails to load
      if (cloudRef.current) {
        const container = d3.select(cloudRef.current);
        container.append("div")
          .attr("class", "fallback-message")
          .style("padding", "20px")
          .style("text-align", "center")
          .style("color", "#ffffff")
          .text("Word cloud visualization couldn't be loaded. Here are some common scam keywords:");
          
        // Display keywords in a simple grid layout
        ...
        SCAM_KEYWORDS.slice(0, 50).forEach(keyword => {
